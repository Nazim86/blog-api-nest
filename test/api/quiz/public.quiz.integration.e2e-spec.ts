...
describe('Super Admin quiz testing', () => {
  ...
  describe('Game connection, answers, return unfinished tests,getting game by id ', () => {
    ...
    it(`Get all my games`, async () => {
      return await getMyStatistic(httpServer, accessTokens[0]);
    });

    it(`Get Top users`, async () => {
      return await getTopUsers(httpServer, accessTokens[0], {
        //pageNumber: 2,
        pageSize: 2,
    ...
    countdown begin and after game ends`, async () => {
      const player1Connect = await connectUserToGame(
        httpServer,
        accessTokens[2],
      );
      const player2Connect = await connectUserToGame(
        httpServer,
        accessTokens[3],
      );

      expect(player2Connect.status).toBe(200);
      expect(player1Connect.status).toBe(200);

      gamePairId = player2Connect.body.id;
